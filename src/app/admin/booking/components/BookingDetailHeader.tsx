    ...
    <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl md:rounded-3xl p-4 md:p-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 md:gap-6">
        ...
        <div className="flex items-start space-x-3 md:space-x-6">
          <div className={`w-12 h-12 md:w-20 md:h-20 rounded-2xl md:rounded-3xl flex items-center justify-center text-white font-bold text-lg md:text-2xl shadow-2xl ${
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-white/60 font-mono text-sm md:text-lg mb-1">#{booking.order_id}</p>
        ...
        <div className="text-left lg:text-right space-y-3 md:space-y-4">
          <span className={`inline-block px-3 py-1 md:px-4 md:py-2 rounded-lg md:rounded-xl text-xs md:text-sm font-semibold border shadow-lg ${getStatusColor(booking.status, booking.payment_status)}`}>
      ...
      {/* Quick Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mt-4 md:mt-8 pt-4 md:pt-6 border-t border-white/10">
        <div className="bg-white/5 rounded-xl md:rounded-2xl p-3 md:p-4 text-center">
          <div className="text-lg md:text-2xl font-bold text-white mb-1">{booking.hours}</div>
          <div className="text-white/60 text-xs md:text-sm">Hours</div>
        </div>
        <div className="bg-white/5 rounded-xl md:rounded-2xl p-3 md:p-4 text-center">
          <div className="text-lg md:text-2xl font-bold text-white mb-1">{booking.lifeguards}</div>
          <div className="text-white/60 text-xs md:text-sm">Lifeguards</div>
        </div>
        <div className="bg-white/5 rounded-xl md:rounded-2xl p-3 md:p-4 text-center">
        ...
        <div className="bg-white/5 rounded-xl md:rounded-2xl p-3 md:p-4 text-center">
          <div className="text-lg md:text-2xl font-bold text-emerald-400 mb-1">
            {booking.payment_status === "paid" ? "✓" : "○"}
          </div>
